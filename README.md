...
- **📡 Sync Nearby** — direct device-to-device sync, paired with a code that's shown as a QR code (scan with the other device's camera) or as plain text — no account, no cloud, no server involved at all
...
- Fully serverless: two devices connect directly to each other over a browser-native peer-to-peer data channel, no signaling server, no account, no cloud in the middle at any point
- One device starts a session and shows a pairing code — as a QR code the other device scans with its camera, or as plain text to copy through any channel you trust; the other device replies the same way and the two connect
- The QR code is generated by a small, from-scratch, dependency-free encoder (`js/qrcode.js`) and scanned via the browser's built-in `BarcodeDetector`; the exact same data is always also shown as copyable text, so a QR code that doesn't scan (older browser, awkward lighting, camera permission denied) never blocks the sync — the text path is the tested, guaranteed-reliable one
- Every message is additionally encrypted with the app's own AES-256-GCM pipeline, keyed on a random secret carried inside the pairing code itself
...
│   ├── lan-sync.js               # Sync Nearby (serverless P2P sync, QR + text pairing codes)
│   ├── qrcode.js                  # From-scratch minimal QR encoder used by lan-sync.js (no dependency/CDN)
...
### v1.11.0 (current)
- **🔄 Sync Nearby rewritten** — pairing is now QR-code-first: one device shows a code as a scannable QR (via a new, from-scratch, dependency-free encoder, `js/qrcode.js`) that the other device reads with its camera (`BarcodeDetector`), instead of only copy-pasting a text blob by hand. The exact same code is always also shown as plain text — a QR code that doesn't scan never blocks the sync, it's a convenience layered on top of the same tested path, not a replacement for it. No server, signaling or otherwise, is involved anywhere in this flow; the two devices connect directly
